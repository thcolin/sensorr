...
The reverse direction matters too: a movie Sensorr believes is `archived` but that Plex no longer has becomes `missing`, and its `from: 'sync'` releases are dropped (`ComputeSensorrMissingMovies` in `apps/cli/src/commands/sync.js`). Two guards keep a Plex read in part from passing for an emptied library. With archived movies in Sensorr, a Plex that lists no movie stops the job with an error before anything is written (`CheckSensorrMoviesTask`). And when more than a quarter of the archived movies would turn `missing` in one run (`LOSS_CEILING`, `apps/cli/src/utils/plex.js`), the job logs one warning and writes none of them. A movie Plex lists but whose metadata request failed is not counted as absent. Unbinding the Plex server pulls every `from: 'sync'` release from every movie for the same reason (`handlePlexReset` in `apps/api/src/app/movies/movies.service.ts`).
...
The units come in a fixed order, and the order is the preference (`libs/sensorr/src/lib/show.ts:198-203`):
...
4. a season pack again, as a last resort, for the seasons that already own an episode, skipped as soon as an episode release of that season was found. That pack is a [season swap](#season-swaps).

One complete release is one download instead of hundreds, and a pack is one per season, so the wider unit comes first. But a pack downloads its whole scope: it is only searched when every episode of that scope is followed and wanted or owned (`asked`, `libs/sensorr/src/lib/show.ts:181`), otherwise a pack would bring back the episodes you unfollowed. A season that owns an episode would get it downloaded twice, which is why its pack only comes last. A unit is not searched at all once the releases picked so far cover it (`isUnitCovered`, `libs/sensorr/src/lib/show.ts:238`), so a whole series found first costs one search per term, and each picked release keeps as its `coverage` only the wanted episodes it is the first to cover (`libs/sensorr/src/lib/show.ts:219`). The last resort pack is the exception: its `coverage` is every episode of its season, the owned ones included, since it replaces them.
...
The policy is the same one, with the same chain, except for two normalizers swapped in when a unit is searched (`libs/sensorr/src/lib/policy.ts:133-135`). `showReleaseUnit` rejects a release whose level, series, season or episode as oleoo reads it, is not the unit's; a multi-season pack counts as the whole series when it holds every season the series wants, so `S01-S03` never stands for ten seasons, and so does a `COLLECTION` once the indexer files it under TV (`levelOf`, `libs/sensorr/src/lib/show.ts:121`). `showReleaseYears` rejects a year outside the show's first and last air years. There is no publish date check. A show that names no policy gets the first one matching its original language, as a movie does (`matchPolicies` in `apps/api/src/app/shows/shows.service.ts:68`).

Every picked release is pushed onto the show's `releases` with its `coverage` and its `level`, one at a time and never by rewriting the array, its id is written on each episode it covers, and its `.torrent` goes to the shows blackhole or to the cache. The release is pushed before its `.torrent` is fetched, so no download reaches the blackhole without its release on the show; a failed download pulls it back and frees its episodes (`apps/cli/src/components/Tasks/ProcessShowsTask.js`). The API reads the file list of that `.torrent` before writing anything and returns it, and the release is patched with it as `torrent`: it is what `import shows` waits for. A `.torrent` it cannot read, or one without a video, is refused with a 422, and the job bans that release on the show and moves on to the next pick (`apps/api/src/app/sensorr/sensorr.service.ts:59`).

`proposal_only` on the show wins when it is set; left empty, the show follows `jobs.record.shows.proposalOnly` (`proposalOnlyOf`, `apps/cli/src/utils/shows.js:31`). A season swap is a proposal whatever either says (`apps/cli/src/components/Tasks/ProcessShowsTask.js:325`). Accept and Refuse on a show release work as on a movie ([Proposals](#proposals)): Accept writes the cached `.torrent` into the shows blackhole and gives the episodes the release id again, Refuse deletes it and takes the id back from the episodes. Both read the proposal back from the database and change that release alone (`upsertShows` in `apps/api/src/app/shows/shows.service.ts`). Accept turns `proposal` off in one write before the download and goes on only when that write changed the release, so a second answer to the same proposal downloads nothing; a failed download turns it back on.

### `airing shows`

Pick up a new episode within the hour it reaches the indexers.

Same component as `record shows`, two differences (`apps/cli/src/commands/airing.js`). It only takes the shows with a `wanted` episode aired in the last seven days (`airing.js:8`, `aired_after` in `apps/cli/src/components/Tasks/ProcessShowsTask.js:34`), and it only searches single episodes, those aired in that window (`airingUnits`, `apps/cli/src/utils/shows.js:50`): hours after an episode airs, there is no pack to find yet. It stands in for an RSS feed, bounded by what aired this week, and leaves the backlog to `record shows`. It reads its own `jobs.airing.shows.proposalOnly`.

### `import shows`

Move finished downloads into the library without copying them.

Takes every release that is not a pending proposal, not imported yet, and whose `.torrent` file list is known (`isImportable`, `apps/cli/src/utils/shows.js:142`). A release is finished when every file of its `.torrent` sits in `shows.staging` at the size the `.torrent` gives, and none also exists with the `.!qB` suffix qBittorrent puts on an incomplete file (`isReleaseFinished`, `apps/cli/src/utils/shows.js:147`). The paths are the torrent's own, a multi-file torrent under a folder named after it (`torrentFiles`, `libs/sensorr/src/lib/torrent.ts:45`), so the download client has to keep that layout.

Each file is read with oleoo for its season and episodes, and linked only when it is a video, the release covers that episode and the episode has no file yet, or the release is a season swap; samples never are (`importLinksOf` in `apps/cli/src/utils/shows.js`). The target is `shows.library/<folder>/<season folder>/<file name>`, the folder being the show's `path`, from Sonarr or an earlier import, or else `<name> (<first air year>)` (`showFolderOf`, `apps/cli/src/utils/shows.js`). The season folder is the one the show folder already has for that season, `Season 1`, `Season 01`, `S1`, `Saison 1`, or `Specials` for season 0, and `Season NN` when there is none (`seasonFolderOf`, same file): a show migrated from Sonarr keeps the `Season 1` Sonarr names by default, instead of getting a `Season 01` next to it. The file keeps its name: Plex's scanner reads it as it is.

A hard link and not a copy or a move: the download client keeps seeding from `.staging`, and the library takes no second copy of the space. It is also why the shows directory is one volume ([architecture.md](architecture.md#one-volume-for-the-hard-link)). The release is stamped `imported_at`, and a show without `path` gets the folder it was linked into (`apps/cli/src/commands/import-shows.js:206-214`). Each linked episode gets the file at once, marked `from: 'import'`, so it reads `owned` without waiting for `sync shows`, which replaces that entry with the file Plex reads, and keeps it while Plex has not scanned the file yet (`importedEpisodesOf` and `plexFilesOf`, `apps/cli/src/utils/shows.js`). A covered episode the release holds no file for, a pack whose files oleoo cannot number or one without a video, gets its `release` cleared and is named in a warning: it reads `wanted` again, and the release is stamped all the same.

An accepted release that never finishes, a failed or stalled download, would keep its episodes `proposed` for good. The rule is the one of a movie swap: a release still not imported a week after its `accepted_at` is marked `overdue: true` (`isReleaseOverdue`, `apps/cli/src/utils/shows.js:144`), and the episodes whose `release` is its id get it cleared, so they read `wanted` and `record shows` searches them again (`apps/cli/src/commands/import-shows.js:189-204`). The episodes are cleared before the mark is written: a run that fails in between clears them again on the next one. The release stays importable, and the mark goes away if it is imported after all. A release downloaded without a proposal is stamped `accepted_at` when it is downloaded (`showReleaseOf`, `apps/cli/src/utils/shows.js`), so it turns overdue the same way. An overdue release is not banned: like a movie swap, it may be picked again.

### `sync shows`

Make Sensorr's idea of your episodes match Plex's.

Refuses to start without a registered Plex server (`apps/cli/src/commands/sync-shows.js:23`). It reads every Plex `show` section, every show with its guids and every episode (`apps/cli/src/commands/sync-shows.js:100-105`). A Plex show is tied to a Sensorr show by its `tmdb://` guid, else by its `tvdb://` or `imdb://` guid against the show's `external_ids`, and several Plex items of one TMDB show are read together (`plexShowOf`, `apps/cli/src/utils/shows.js`). One tied by none of them is logged and skipped; when its title and year still name a Sensorr show, that show keeps its files instead of losing them as absent from Plex (`:148-159`). Episodes are matched on Plex's season `parentIndex` and episode `index` against TMDB's numbers (`showFilesOf`, `apps/cli/src/utils/plex.js:130`); a Plex episode TMDB numbers differently is counted as `unmatched` and left out, since TMDB alone decides the numbering. Each file is kept on its episode with its size and a name built from its streams, as `sync` builds a movie's (`fileOf`, `apps/cli/src/utils/plex.js`). A section listing carries no streams, so a file Sensorr has not read yet costs one metadata request on its episode; a file already read keeps its name, and a library of thousands of episodes pays those requests once, not on every run (`unreadItemsOf`, `apps/cli/src/utils/plex.js`, and `apps/cli/src/commands/sync-shows.js`).

A show Plex has and Sensorr does not is added `archived` and unfollowed, episodes included (`apps/cli/src/commands/sync-shows.js:182-190`): you have it, nothing should search it. An episode that loses a file Plex had seen logs one warning per show, which is also a notification and a web push (`NOTIFYING` in `apps/api/src/app/notifications/notifications.service.ts`), and a Sensorr show absent from Plex loses the files of every episode, but for the ones the import linked and Plex has not scanned yet. Every episode left without a file waits for the last task, which writes the losses of the whole run or none of them (`ComputeSensorrMissingEpisodesTask`). Two guards keep a Plex read in part from passing for an emptied library: a Plex that lists no episode while Sensorr episodes hold a file from it stops the job with an error before anything is written, and when more than a quarter of those episodes would lose their files in one run (`LOSS_CEILING`, `apps/cli/src/utils/plex.js`), the job logs one warning and writes none of them. The show keeps its state, there is no `missing` show. An episode that loses its files loses its `release` id too (`syncedFilesOf`, `apps/cli/src/utils/shows.js:70`): it reads `wanted` again, and `record shows` searches it. Unbinding the Plex server leaves the episode files as they are, as a movie stays `archived`: nothing on the shows side listens to `plex.reset`, the next `sync shows` sets them right, and until then no job searches what you own.

### Season swaps

The last resort pack of `record shows` fills the episodes of a season that no single episode release was found for, and brings the ones you own with it: two copies of each on Plex. So it is a swap, the series side of a `refine` or `shrink` one, and ends the same way ([`sync`](#sync)).

It is stored with `swap: true` and a `coverage` holding every episode of its season (`pickReleases`, `libs/sensorr/src/lib/show.ts`), and it is always a proposal, even on a show set to download at once: it replaces files you have, like `refine` and `shrink` whose `proposalOnly` ships on. The proposal row on the show page and the notification say it: `14.9 GB replaces 19 episodes (6.1 GB) and fills 1`, the size of the release, the owned episodes of its season and the size of their files, and the episodes it adds (`swapOf`, same file). While it waits for an answer, `sync shows` withdraws it once every episode it covers has a file, as any show proposal: it would fill nothing.

Accepting it keeps `replaces`, the Plex ids of the files of that season at that moment, and `accepted_at` (`swapReplacesOf`, same file, called from `upsertShows` in `apps/api/src/app/shows/shows.service.ts`). A file `migrate sonarr` or `import shows` wrote is not a Plex id and is never among them: nothing but Plex can delete it. `import shows` then links every episode file of the pack, the owned episodes included, and writes its `from: 'import'` entry next to the files each episode already has. A target that already exists is logged and left, as for any release.

`sync shows` ends it once every covered episode has, on Plex, a version that is not in `replaces` and that is one of the files of its `.torrent`, same name and same size (`settleSeasonSwaps`, `apps/cli/src/utils/swaps.js`). The import links a file under its own name, so the match is exact, where a movie swap has only the size the indexer announced to go by. With `jobs.sync.shows.cleanup` on, the same run deletes the replaced versions still on Plex through `DELETE /library/metadata/{ratingKey}/media/{mediaId}`, the endpoint `sync movies` uses, then ends the swap by emptying `replaces`. A 403, the Plex server not allowing media deletion, is reported under warnings, and the swap stays open for the next run. With it off, the swap ends as soon as it lands and nothing is deleted. Nothing is deleted before every covered episode has its new version on Plex, so a pack that misses one episode, or holds a file oleoo cannot number, never lands and deletes nothing. A swap not landed a week after it was accepted is marked `overdue: true`, and the mark goes away if it lands later.

### `refresh shows`

Keep TMDB metadata of shows from going stale, and learn the episodes TMDB adds.

A show still airing, TMDB `status` `Returning Series`, `In Production`, `Planned` or `Pilot`, is refreshed on every run; any other once its last refresh is 30 days old (`isRefreshDue`, `apps/cli/src/utils/shows.js:12`). An airing show gets episodes every week, an ended one rarely changes, and a refresh costs one TMDB request per 20 seasons (`libs/tmdb/src/shows.ts:76-92`). It reads the `ignored` shows too (`fetchSensorrShows`, `apps/cli/src/utils/shows.js:89`), so a requested show stays current.
