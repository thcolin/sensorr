...
export const fields = {
  ...
  original_languages: {
    initial: { values: [], behavior: 'or' },
    serialize: serializers.select('multi'),
    statistics: statisticians.array('original_languages', true),
  },
  spoken_languages: {
    initial: { values: [], behavior: 'or' },
    serialize: serializers.select('multi'),
    statistics: statisticians.array('spoken_languages', true),
  },
  production_companies: {
    initial: { values: [], behavior: 'or' },
    serialize: serializers.select('multi'),
    statistics: statisticians.array('production_companies', true),
  },
  companies: {
    initial: { values: [], behavior: 'or' },
    serialize: serializers.select('multi'),
  ...
  budget: {
    ...
    serialize: serializers.range(200),
    statistics: statisticians.boundaries('budget'),
    humanize: ({ budget = 0 }: Movie) => `${commarize(budget)}M$`,
  },
  runtime: {
    initial: [0, 240],
