...
describe('pushOf', () => {
  ...
  it('names a show release by the show and the episodes it covers', () => {
    ...
    expect(pushOf({ command: 'record-shows', show, release: { ...release, coverage: range(3, 1, 25), meta: { type: 'tvshow', seasons: [3], episodes: [] } } })).toEqual({
      title: 'Friends S03',
      body: '📹 C411, 1 GB, 3 peers\nFriends.S03.MULTi.1080p.BluRay.x264-GRP',
      image: 'https://image.tmdb.org/t/p/w185/friends.jpg',
      actions: accept,
    })
    expect(pushOf({ command: 'airing', show, release: { ...release, coverage: range(3, 4, 6), meta: { type: 'tvshow', seasons: [3], episodes: [4, 5, 6] } } }).title).toBe('Friends S03E04-E06')
    expect(pushOf({ command: 'airing', show, release: { ...release, coverage: range(3, 4, 6), meta: { type: 'tvshow', seasons: [3], episodes: [4, 5, 6] } } }).body).toBe('📡 C411, 1 GB, 3 peers\nFriends.S03.MULTi.1080p.BluRay.x264-GRP')
    expect(pushOf({ command: 'record-shows', show, release: { ...release, coverage: [...range(1, 1, 24), ...range(10, 1, 18)], meta: { type: 'tvshow', seasons: [1, 10], episodes: [] } } }).title).toBe('Friends S01-S10')
  })

  it('offers no answer to a show release already downloaded', () => {
    expect(pushOf({ command: 'record-shows', show: { id: 1668, name: 'Friends' }, release: { title: 'Friends.S03E04.1080p.WEB.x264-GRP', coverage: [{ season: 3, episode: 4 }], proposal: false, valid: true } })).toMatchObject({
  ...
  it('names a show request by the show alone', () => {
    expect(pushOf({ command: 'keep-in-touch', show: { id: 1668, name: 'Friends', poster_path: '/friends.jpg' }, requested_by: ['a@example.com'], processed: true })).toEqual({
      title: 'Friends',
