...
export const summary = ({ shrinked = 0, processed, recorded = 0, proposal = 0, treated = 0, withdrawn, ignored, missing, warning }, extended = true, config = {} as any) => [
  ...(extended ? [{
    key: 'shrinked',
    emoji: '💎',
    title: <span><strong>{shrinked}</strong> Shrinkable movies</span>,
    length: shrinked,
  }] : []),
  ...
  ...(config?.proposalOnly && (treated > 0) ? [{
    key: 'treated',
    emoji: '💍',
    title: <span><strong>{treated}</strong> Shrinked movies</span>,
    length: treated,
  }] : [{
    key: 'recorded',
    emoji: '💍',
    title: <span><strong>{recorded}</strong> Shrinked movies</span>,
    length: recorded,
  }]),
